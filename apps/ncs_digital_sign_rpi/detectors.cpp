...
// Search "intel" for false boolean variables added for data extract
...
AgeGenderDetection::Result AgeGenderDetection::operator[] (int idx) const {
    ...
    if (intel2) {
    	if (r.maleProb > 0.70){
    		//std::cout << "[OUTPUT] = Male Advertisement" << std::endl;
    		//std::cout << "[Age] = " << r.age << std::endl;
    	} 
        if (r.maleProb < 0.30){
            //std::cout << "[OUTPUT] = Female Advertisement" << std::endl;
            //std::cout << "[AGE] = " << r.age << std::endl;
        }
...
std::map<std::string, float> EmotionsDetection::operator[] (int idx) const {
    ...
    for (size_t i = 0; i < emotionsVecSize; i++) {
        ...
        if (doRawOutputMessages) {
            std::cout << "[EMOTION] " << emotionsVec[i] << std::endl;
