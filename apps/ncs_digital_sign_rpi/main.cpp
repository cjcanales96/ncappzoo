...
Config loadConfig(){
   ...
   /* FILE* fp = fopen("config.json", "rb");
	...
	assert(d.HasMember("maleAdvertisement"));

	assert(d["maleAdvertisement"].IsString());

	std::string a = d["maleAdvertisement"].GetString();

	config.maleAdv = a;

	assert(d.HasMember("femaleAdvertisement"));

	assert(d["femaleAdvertisement"].IsString());

	std::string b = d["femaleAdvertisement"].GetString();

	config.femaleAdv = b;

	assert(d.HasMember("liveFeed"));

	assert(d["liveFeed"].IsString());

	std::string h = d["liveFeed"].GetString();
...
void Visualizer::showImg(int age, int gender) {
	...
	if (gender == 1) {
		if (age < 17) {
			cv::Mat image = cv::imread("/home/pi/Documents/male_young1.png");
			cv::imshow("Advertisement", image);
			cv::waitKey(1);
		}
		else if (age > 17 && age < 35) {
			cv::Mat image = cv::imread("/home/pi/Documents/male_old1.png");
			cv::imshow("Advertisement", image);
			cv::waitKey(1);
		}
		...
		if (age < 17) {
			cv::Mat image = cv::imread("/home/pi/Documents/female_young1.png");
			cv::imshow("Advertisement", image);
			cv::waitKey(1);
		}
		else if (age > 17 && age < 35) {
			cv::Mat image = cv::imread("/home/pi/Documents/female_old1.png");
			cv::imshow("Advertisement", image);
			cv::waitKey(1);
		}
		else if (age > 35 && age < 50) {
        ...
        while (true) {
            ...
            if (!FLAGS_no_show || !FLAGS_o.empty()) {
				...
				if (!FLAGS_no_show) {
					...
					//std::cout << "CONFIG1 = " << config.fullscreen << std::endl;

					if (config.liveFeed == "yes") {
						if (flag == true) {
							cv::namedWindow("Advertisement", 0);
							cv::moveWindow("General Advertisement", 20, 20);
							cv::setWindowProperty("Advertisement", 0, 1);
							cv::Mat ad = cv::imread("/home/pi/Documents/gender_neutral1.jpg");
							cv::imshow("Advertisement", ad);
							flag = false;
						}

						cv::namedWindow("Detection Results");
						cv::moveWindow("Detection Results", 0, 0);
